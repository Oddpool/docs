...
OHLCV bars for every outcome under an event. Use this for charting an event's outcomes side-by-side: FOMC rate decisions, NBA championship candidates, election nominees, NFL game spreads.

For a single market, an arbitrary set of markets, or one specific outcome across multiple events, use [Market OHLCV](/markets/ohlcv) instead.
...
## Example — FOMC June rate decision dashboard
