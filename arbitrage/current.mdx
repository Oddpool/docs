...
<ParamField query="orderbook" type="boolean" default="false">
  When `true`, attaches each leg's live ask ladder (top 10 levels) under `orderbook`, so you can size an order without a second call.
</ParamField>
...
The opportunity tells you the trade directly: **buy YES on `buy_yes_market`, buy NO on `buy_no_market`.** `yes_leg` / `no_leg` give the exact `(market, identifier, book_side)` to hit for each side, and `executable_size` / `max_profit_dollars` estimate how much you can fill and earn at the current depth.
...
<Note>
`orderbook` is only present when you pass `orderbook=true`. It carries the top 10 ask levels per leg — ask side only (the price ladder to *buy* that outcome). Token IDs above are truncated for readability; the API returns them in full.
</Note>
