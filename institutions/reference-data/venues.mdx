...
- **`venue`** — [Get Oddpool ID by ticker](/institutions/reference-data/events/lookup) takes a single tag alongside the venue's `id`.
- **`venue_pair`** — [List matched events](/institutions/reference-data/events/list) takes two tags to scope the fungibility filters to one pair.
