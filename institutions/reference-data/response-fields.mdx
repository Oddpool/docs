...
<ResponseField name="venue_market_id" type="string">
  The venue's native identifier for this tradeable side — what you trade on.

  - Kalshi: the market ticker.
  - Polymarket binary YES/NO: the `conditionId`.
  - Polymarket atomic-binary multi-side: `conditionId#<index>`. Split on `#` to recover the native `conditionId`, then subscribe to the side with the tokens below.
</ResponseField>
<ResponseField name="venue_event_id" type="string | null">The venue's identifier for the parent event.</ResponseField>
<ResponseField name="venue_yes_token" type="string | null">Polymarket CLOB token id for the YES side. `null` on Kalshi.</ResponseField>
<ResponseField name="venue_no_token" type="string | null">Polymarket CLOB token id for the NO side. `null` on Kalshi.</ResponseField>
<ResponseField name="venue_question" type="string | null">Raw question text from the venue at the time of mapping.</ResponseField>

## Outcome venue pair

An entry in an outcome's `venue_pairs` — whether the outcome's two venue sides are one tradeable instrument.
...
<ResponseField name="divergence_axes" type="string[]">
  ...
  | `condition_scope` | Same nominal outcome, but the venues resolve an edge case to different cash settlements — e.g. one pays fair/last-traded price on a cancelled or postponed event while the other resolves No or 50-50, or they differ on what counts toward the condition. Often reflects standard cancellation/postponement policy; common on sports and entertainment. |
</ResponseField>
...
<ResponseField name="metadata" type="object">
  Kind-specific structured fields. Shape varies by `kind` — e.g. Person: `country`, `party_qid`, `person_kind`; Team: `league`, `sport`, `city`, `parent_qid`; Organization: `country`, `tickers`. Values that reference other entities are Wikidata Q-IDs (`country: "Q30"`). Treat as open JSON: use the keys you recognize, ignore the rest.
</ResponseField>
